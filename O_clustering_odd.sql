...
BEGIN
    FOR i IN 1..100 LOOP
        ...
        -- first cluster
        Insert into cluster_test VALUES(CASE_ID_SEQ.nextval,round(dbms_random.value(5, 25)),round(dbms_random.value(5, 25)));
        
        -- second cluster
        Insert into cluster_test VALUES(CASE_ID_SEQ.nextval,round(dbms_random.value(75, 95)),round(dbms_random.value(75, 95)));
        
        -- third cluster
        Insert into cluster_test VALUES(CASE_ID_SEQ.nextval,round(dbms_random.value(75, 95)),round(dbms_random.value(5, 25)));
        
         -- fourth cluster
        Insert into cluster_test VALUES(CASE_ID_SEQ.nextval,round(dbms_random.value(5, 25)),round(dbms_random.value(75, 100)));
    END LOOP;
    commit;
END;
...
SELECT case_id,
  ...
  FROM cluster_test
 WHERE case_id <= 100005
 ORDER BY case_id;


SELECT t.*, CLUSTER_PROBABILITY (OC_SH_Clus_sample,2  USING *)
    FROM cluster_test t; 
